# Postmypost\RestSdk\TimezonesApi

All URIs are relative to https://api.postmypost.io/v4.1, except if the operation defines another base path.

| Method | HTTP request | Description |
| ------------- | ------------- | ------------- |
| [**getTimezones()**](TimezonesApi.md#getTimezones) | **GET** /timezones | Get list of timezones |


## `getTimezones()`

```php
getTimezones($sort, $page, $per_page): \Postmypost\RestSdk\Model\TimezonesResponse
```
...
$apiInstance = new Postmypost\RestSdk\Api\TimezonesApi(
...
try {
    $result = $apiInstance->getTimezones($sort, $page, $per_page);
    print_r($result);
} catch (Exception $e) {
...
[**\Postmypost\RestSdk\Model\TimezonesResponse**](../Model/TimezonesResponse.md)
