# Postmypost\RestSdk\RubricsApi

All URIs are relative to https://api.postmypost.io/v4.1, except if the operation defines another base path.

| Method | HTTP request | Description |
| ------------- | ------------- | ------------- |
| [**getRubrics()**](RubricsApi.md#getRubrics) | **GET** /rubrics | Get list of rubrics |


## `getRubrics()`

```php
getRubrics($project_id, $sort, $page, $per_page): \Postmypost\RestSdk\Model\RubricsResponse
```
...
$apiInstance = new Postmypost\RestSdk\Api\RubricsApi(
...
try {
    $result = $apiInstance->getRubrics($project_id, $sort, $page, $per_page);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling RubricsApi->getRubrics: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\Postmypost\RestSdk\Model\RubricsResponse**](../Model/RubricsResponse.md)
